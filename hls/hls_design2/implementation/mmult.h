...
template <typename T, int DIM>
void mmult_hw(T a[DIM][DIM], T b[DIM][DIM], T out[DIM][DIM], int quadrant)
{
	int ial, iau, ibl, ibu;
...
#pragma HLS PIPELINE II=1
			...
			// do HW multiplication
				mmult_hw<T, DIM>(a,b,out, 1);
				mmult_hw<T, DIM>(a,b,out, 2);
				mmult_hw<T, DIM>(a,b,out, 3);
				mmult_hw<T, DIM>(a,b,out, 4);
